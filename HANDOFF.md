...
26. Double elimination is allowed as an optional generated format for smaller tournaments.
27. Double elimination should keep both brackets moving, but should avoid sending someone straight from a winners-bracket loss into their next losers-bracket match when other matches are available.
...
Double elimination is supported as a real tournament format in the engine.

Setup has an **Allow double elimination** checkbox. When it is enabled and the player count is 4 through 17, the generated options can include a Double Elimination option.

Current double-elimination behavior:

- All double-elimination matches are first to 11.
- The bracket includes a winners bracket, losers bracket, and one championship match.
- The app does not create a visible bracket-reset match.
- The estimate counts a possible reset as an extra match, using `2N - 1` total matches.
- The championship note tells the organizer that if the losers-bracket winner wins the championship, play one additional 11-point deciding game.

This keeps the on-screen bracket simpler while still planning time honestly.
...
When double elimination is allowed for 4-17 players, it may replace the Relaxed option as the third generated option.
...
- For normal single elimination under normal player counts, the championship/final is first to 21.
- For normal single elimination under normal player counts, semifinals are at least first to 15.
- If player count is over 28, Fast / Safest can use 11-point games all the way through the championship.
- If player count is over 28, Balanced can use 11-point semifinals when the timing math needs it.
...
- Double Elimination uses all 11-point games and is only generated from 4-17 players when allowed.
...
For single-elimination tournaments, this original ready-match ordering should stay unchanged.

For double-elimination tournaments, ready-match ordering has a separate scheduling path:

- losers bracket matches are mixed in as soon as they are ready
- losers bracket matches are generally preferred over winners bracket matches once enough rest exists
- a player who played in either of the last two completed matches is deprioritized
- a player who just lost from the winners bracket into a losers bracket match gets an additional rest penalty
- the rest rules are soft, so if the tournament would otherwise stall, the available match can still rise to the top

This is meant to keep both brackets moving while avoiding unnecessary back-to-back table hops early in the tournament.
...
Retrieve Database intentionally preserves local-only device fields:

- current device role (`master` or `viewer`)
- current active tab/view

This prevents a helper iPad from becoming a control device or jumping views just because it pulled the master snapshot.
